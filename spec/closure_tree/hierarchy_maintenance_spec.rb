...
RSpec.describe ClosureTree::HierarchyMaintenance do
  describe '.rebuild!' do
    ...
    before do
      samples_count.times do |counter|
        Metal.create(:value => "Nitro-#{counter}", parent: Metal.all.sample)
      end
    end

    it 'the hierarchy nodes should be at least one per model' do
      expect(MetalHierarchy.count).to be >= samples_count # shallowest-possible case, where all children use the first root
    end
